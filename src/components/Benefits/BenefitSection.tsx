...
const BenefitSection: React.FC<Props> = ({ benefit, index, imageAtRight }: Props) => {
            ...
            >
                    ...
                    <div className="w-full  text-center lg:text-left ">
                        ...
                        <div className="mx-auto lg:ml-0 w-full">
                            {bullets.map((item, bulletIndex) => (
                                <BenefitBullet
                                    key={bulletIndex}
                                    title={t(`items.${index}.bullets.${bulletIndex}.title`)}
                                    icon={item.icon}
                                    description={t(`items.${index}.bullets.${bulletIndex}.description`)}
                                />
                            ))}
                        </div>
                    </div>
                ...
                <div className={clsx("mt-5 lg:mt-0", { "lg:order-2": imageAtRight })}>
                    <div className={clsx("w-fit flex", { "justify-start": imageAtRight, "justify-end": !imageAtRight })}>
                        <Image src={imageSrc} alt={t(`items.${index}.title`)} width="384" height="762" quality={100} className="lg:ml-0" />
                    </div>
                </div>
            </motion.div>
        </section>
